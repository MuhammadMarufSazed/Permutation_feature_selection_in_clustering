...
mclust_vals = feature_importance(data = as.matrix(data), no_shufle = 30, cluster_fn = mclust_fn)

kmeans_vals = feature_importance(data = as.matrix(data), no_shufle = 30, cluster_fn = kmeans_fn)
```

```{r}
mclust_original_data = mclust_fn(data, G = 2, verbose = FALSE)
kmeans_original_data = kmeans_fn(data, G = 2)

im_mclust = (mean_mclust_perm - mclus_irisdata$loglik)/mclus_irisdata$loglik
